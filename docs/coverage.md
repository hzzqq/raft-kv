...
| `check_leaked_artifacts.py` | 根目录 `covtest_*` / `*.out` / `*.test` / `tmp_*` 临时产物泄漏护栏，未忽略即硬失败，并校验 `.gitignore` 含 `covtest_*/` 根因规则 |
...
|-----------|---------:|--------:|:------:|---------------------:|
| `cluster` | 3 | 7 | ✅ | CrashNode, Diagnostics, LoadTCPConfig, ProposeConfChange, StartClusterFromConfig, VoterConfig, NodeDiagnostics |
...
| `raft` | 4 | 16 | ✅ | Call, CondInstallSnapshotWithConfig, HasPendingConf, MakeSendFnEnd, PartitionServers, RaftStateSize, Send, SetSendFn …(+2) |
...
> 汇总：17 个包，1 个无测试；未引用导出符号 func=32 / type=19 / var=2 / const=0。「未覆盖符号」为软提示，可能含被间接覆盖的结果/视图类型。
