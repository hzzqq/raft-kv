...
- **`gateway`**（`src/gateway`）：自带进程内集群的 HTTP REST 网关，`GET/PUT/POST-append /kv/{key}`、
  ...
  另提供 **并发限流**（默认 64 并发，`sem` 信号量，超量返回 `429 Too Many Requests`，避免网关在迁移抖动期被压垮）；**优雅退出 `Shutdown(ctx)`**（`sync.WaitGroup` 等待在途请求结束、关闭 `applyCh` 防泄漏，供 `main.go` 的 `SIGINT`/`SIGTERM` 调用）；新增 **`GET /debug/groups`** 暴露各 replica group 的 `gid` / 副本数 / leader / 当前 config / 持有分片数，便于排障，  配套 `TestGatewayConcurrencyLimit` / `TestGatewayGroups` 回归。完整端点表（含 `/debug/raft`、`/debug/migrate-plan`、`/debug/routes`、`/debug/version`、`/debug/configs`、`/debug/config`、`/debug/accesslog`、`/debug/log`）见 [`docs/architecture.md` §2 层级职责一览](docs/architecture.md)。
...
> 本机复跑入口：`make docs`（等价于 CI `docs-links` job，免 Go）；`make ci`（等价于 CI `gate` job，跑统一编排器 `check_all.py` + 校验器自身回归，免 Go）；`make hooks` 安装提交前门禁，从根上阻止文档漂移 / CHANGELOG 失配 / 日志污染被提交进树。需要机器可读结果（CI / 看板消费）时可用 `python3 scripts/check_all.py --json`，输出含 `summary` 与逐条 `checks` 的 JSON 报告。性能回归门禁：`make bench`（需 Go）跑全包热点路径基准并经 `scripts/check_bench_regression.py` 比对 `scripts/bench-baseline.json`，回退 >10% 即失败；`make bench-update` 在「确认性能变化有意为之」时刷新基线（CI `bench-guard` job 已接通该护栏）。
...
### R6/R7 自主迭代交付（#177–#211，弹性/传输层/正确性双修/可观测）

全部 cluster-free 验证 + 本地提交，沿用「网关/客户端/工具测试绕开进程内 raft 选举」纪律：

- **util 三件套（#177–#182）**：`TimedCache` 带 TTL 并发安全缓存（注入时钟、惰性过期 + 主动 GC）、`SnowID` Snowflake 风格 ID 生成器（10 位节点 + 12 位序列、时钟回拨防护）、`JSONError` 结构化错误（实现 `error` + 定制 `MarshalJSON`，details 零值省略）。
- **kvcli 弹性组合拳（#183–#184、#188–#190、#192）**：`EnableGzip` 响应压缩协商、`Ping`/`Healthy`/`Ready` 探活（对接网关 `/healthz`、`/readyz`）、503/504 尊重 `Retry-After` 头（覆盖默认退避）、`EnableBreaker` 熔断（复用 `util.CircuitBreaker` 快失败）、`Metrics`/`AvgLatency` 客户端观测、`Close()` 优雅回收空闲连接；重试退避移入失败分支（成功路径零延迟）。
- **gateway（#185–#187、#191）**：限流响应头 `X-RateLimit-Limit/Remaining/Reset`（`tokenBucket.inspect` 不消耗令牌）、`Shutdown` 一并关闭 gRPC 监听（修复端口泄漏）、回写 `X-Request-Id` 响应头（跨服务链路关联）。
- **shardkv（#180 附）/shardmaster（#181 附）/diagnostics（#182 附）**：`ShardData` Snapshot/Restore/Merge/Subtract（gob 编码、去重身份随迁）、`Config.Valid` + `IsValidTransition`（Num 严格 +1 且结构合法）、`SelfCheck` 配置链自检（跳号/孤儿扣分）。
- **transport 传输层增强（#193–#195）**：`Server.SetIdleTimeout` 读空闲超时（回收半开/慢速连接占用的 goroutine，修复连接泄漏隐患）、`ClientConn.Warmup` 连接池预热（消除首 RPC 建链尖刺）+ `SetDialTimeout` 建链超时可配、`GobCodec` 二进制编解码 + 泛型 `TypedHandler`（消除服务端字节编解码样板）+ `ServiceBuilder` 链式注册 + `SetCodec`（修复 codec 无锁读写竞态）。
- **statusfmt（#196）**：主流程抽 `run()` 可测核心；健康/均衡评分**接入输出**（此前为从未被调用的准死代码）；新增 `-json` 机器可读报告；STALLED 集群退出码 2（供脚本/CI 探活，此前恒 0）。
- **version + gateway 接入（#197）**：修复 version 包全库零引用的死代码状态——`/debug/version` 未 SetVersion 时回退构建期注入的 `BuildVersion`，新增 `commit`/`build_time` 字段；version 增 `IsDev()`/`Short()`。
- **kvraft 快照正确性双修（#198）**：修复 applier 对 `appliedIndex` 的无锁写竞态；修复陈旧快照（`SnapshotIndex <= appliedIndex`）无条件安装导致状态机回滚、破坏线性一致的隐患（持锁拒绝）；nil map 归一防 panic。
- **文档同步（#199）**：补齐 R6/R7 交付段，修复近 22 轮新能力零文档记录（文档漂移）。
- **gateway 处理/响应/请求可观测头（#200、#205、#208）**：`X-Process-Time` 服务端处理耗时（TTFB，毫秒三位小数，外层 writer 规避 net/http 头冻结）；`X-Response-Size` 响应体线上字节数（直方图指标 `gateway_response_bytes`）；`X-Request-Size` 入站请求体声明大小（分块 `-1` 跳过）。三者均经 `wrap` 统一注入，所有路由自动受益。
- **transport 便捷注册 + 优雅关闭（#201、#204）**：包级泛型 `RegisterFunc` 把类型化业务函数直接挂方法名（内部 `TypedHandler` 默认 JSON，省样板）；`serveConn` 新增 `safeCall` panic 恢复（单 handler 崩溃不再拖垮服务端）；`GracefulStop` 停监听后等待在途 RPC 清空再退出（滚动发布先排空），`InFlight()` + `ServerMetrics.InFlight` 暴露在途数。
- **shardmaster 配置变更预览器（#202）+ 网关 dry-run 端点（#206）**：纯函数 `Plan(current, PlanOp)` 在内存模拟 Join/Leave/Move 并复刻 `applyOp` 语义，用已有 `Valid`/`IsValidTransition` 双校验，返回 `Planned`/`Errors`/`TransitionErr`/`Moves`（不修改入参）；网关 `POST /debug/migrate-plan` 将其暴露为 HTTP，运维提交前安全评估迁移代价与风险，避免非法配置致 rebalance 卡死（复用既有 `GET /debug/migrate` 运行时状态不冲突）。
- **kvcli 批量并发收敛（#203、#207）**：`SetMaxConcurrent(n)` 限制 `MGet`/`MSet` 并发回源 goroutine 数（默认不限制，向后兼容）；实现由手搓裸 channel 收敛复用 `util.Semaphore`（支持 ctx 取消，满信号量 + ctx 取消立即返回错误不再挂死）。
